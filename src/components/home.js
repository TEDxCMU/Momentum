...
function Home() {
    ...
    const [imageIndex, setImageIndex] = useLocalStorage('momentum_image_index', Math.floor(Math.random() * 6) + 1);
    ...
    function handleColorChange(event) {
        ...
        let newColor = pastColor;
        do {
            newColor = Math.floor(Math.random() * colors.length);
        } while (newColor === pastColor);

        pastColor = newColor;

        for (const path of paths) {
            path.setAttribute('stroke', colors[pastColor]);
        }
    }
    ...
    useEffect(() => {
        ...
        do {
            newImageIndex = Math.floor(Math.random() * 6) + 1;
